...
const spriteWin = @import("sprites.zig").spriteWin;
...
pub fn main() !void {
    ...
    var bothPlayers = BothPlayers.init(64, 32, 5);
    ...
    var scene: enum { game, p1_win, p2_win } = .game;
    ...
    while (!rl.windowShouldClose()) {
        ...
        switch (ball.update(bothPlayers)) {
            .p1_score => {
                p1.scores();
                if (p1.score == winningScore) scene = .p1_win;
                service = bothPlayers.getServeSide();
                ball.newRound(service, bothPlayers);
            },
            .p2_score => {
                p2.scores();
                if (p2.score == winningScore) scene = .p2_win;
                service = bothPlayers.getServeSide();
                ball.newRound(service, bothPlayers);
            },
            else => {},
        ...
        // add ball
        try display.addDotVec(ball.position, rl.Color.yellow);
        // add players
        try display.addRectangleRec(p1.getRectangle(), rl.Color.red);
        try display.addRectangleRec(p2.getRectangle(), rl.Color.blue);
        // add "WIN"
        switch (scene) {
            .p1_win => try display.addSpriteWith(@TypeOf(spriteWin), 6, 1, spriteWin, rl.Color.green),
            .p2_win => try display.addSpriteWith(@TypeOf(spriteWin), 46, 1, spriteWin, rl.Color.green),
            else => {},
        }
        // draw everything
        try display.draw();
